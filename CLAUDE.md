...
- Не коммитить крупный этап (новый шаблон, архитектурное изменение) без явного одобрения пользователя и ревью по скиллу `code-review-expert` (`.claude/skills/code-review-expert`) — см. `## Project State`.
